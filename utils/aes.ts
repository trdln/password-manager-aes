...
class Aes {
  ...
  static cipher(input: number[], w: number[][]): number[] {
    ...
    let state: number[][] = [[], [], [], []]; // initialise 4×Nb byte-array 'state' with input [§3.4]
   ...
   * @private
   */
  static mixColumns(s: any, Nb: any) {
    for (let c = 0; c < Nb; c++) {
   ...
   * @private
   */
  static addRoundKey(state: number[][], w: any, rnd: any, Nb: any) {
    for (let r = 0; r < 4; r++) {
...
export class AesCtr extends Aes {
   ...
   */
  static encrypt(plaintext: string, password: string, nBits: 128 | 192 | 256): string {
   ...
   * @private
   */
  static nistEncryption(plaintext: number[], key: number[], counterBlock: number[]): number[] {
   ...
   */
  static decrypt(ciphertext: string, password: string, nBits: 128 | 192 | 256): string {
    if (![128, 192, 256].includes(nBits)) throw new Error('Key size is not 128 / 192 / 256');
   ...
   * @private
   */
  static nistDecryption(ciphertext: number[], key: number[], counterBlock: number[]): number[] {
    const blockSize = 16; // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
